...
public:
    PathRecorder() : Node("path_recorder")
    {
        ...
        gt_output_file_ = this->get_parameter("gt_output_file").as_string();
        ...
        RCLCPP_INFO(this->get_logger(), "  GT topic: %s -> %s", gt_topic.c_str(), gt_output_file_.c_str());
    ...
    ~PathRecorder()
    {
        ...
        if (gt_file_.is_open()) {
            ...
            RCLCPP_INFO(this->get_logger(), "Saved GT trajectory with %zu poses to %s", 
                       gt_pose_count_, gt_output_file_.c_str());
        }
    }
